...
class Ant(env.Env):
  ...
  * v0: Initial versions release (1.0.0)
  ...
  def _get_obs(self, qp: brax.QP) -> jp.ndarray:
    ...
    barrier_bool = [jax.lax.cond(qp.pos[14,2] < 0, return_cage_down, return_cage_up)]

    return jp.concatenate(qpos + qvel + barrier_bool)
...
_SYSTEM_CONFIG = """
  ...
  bodies {
    name: "South_Wall"
    colliders {
      box { halfsize { x: 9.4 y: 1.2 z: 0.75} }
  ...
  bodies {
    name: "Button"
    colliders {
      box { halfsize { x: 2.05 y: 2.05 z: 0.01} }
      position {x: -5.05 y: -5.05 z: 0.01}
    }
    frozen { all: true }
  }
  joints {
